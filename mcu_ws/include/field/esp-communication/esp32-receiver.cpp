...
// if by choice find and set the mac address to a public array that contains the
// hexadecimals for the MAC address
...
// if desired to get the actual mac address user will input their own array to
// have a "copy" of the MAC address of the reciever
...
// using our reciever structure which contains the atenna number we can make a
// list of attenas that 1: completed task  0: not completed
void Reciever::checkAtenna() { atennas[recieverend.atenna_number - 1] = 1; }
// uses protocols from ESP32 library and then copies the strucutre message into
// the reciever structure
void Reciever::onDataRecv(const uint8_t* mac, const uint8_t* incomingData,
                          int len) {

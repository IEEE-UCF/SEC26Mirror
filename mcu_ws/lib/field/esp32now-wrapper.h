...
namespace RawDrivers{
  ...
  class EspNow:public Classes::BaseDriver{
    ...
    ~EspNow() override = default;
      EspNow(const ESPNowSetup &setup) : Classes::BaseDriver(setup), _setup(setup){}
      //@breif intialize and set up esp for receving and sending signals with one another including adding peers
      bool init() override;
      //@brief one of the message structs contain the score of which earth will send this is how it'll do it

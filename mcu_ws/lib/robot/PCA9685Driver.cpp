...
void PCA9685Driver::writeDigital(uint8_t channel, bool on) {
  ...
  I2CBus::Lock lock(setup_.wire_);
  if (on) pwm_.setPWM(channel, 4096, 0);
  else    pwm_.setPWM(channel, 0, 4096);
}

void PCA9685Driver::writePWM(uint8_t channel, uint16_t duty) {
  if (channel > 15) return;
  I2CBus::Lock lock(setup_.wire_);
  if      (duty >= 4095) pwm_.setPWM(channel, 4096, 0);
  else if (duty == 0)    pwm_.setPWM(channel, 0, 4096);
  else                   pwm_.setPWM(channel, 0, duty);
}
...
void PCA9685Driver::applyBuffered() {
  ...
  for (uint8_t ch = 0; ch < 16; ++ch) {
    ...
    if      (duty >= 4095) pwm_.setPWM(ch, 4096, 0);
    else if (duty == 0)    pwm_.setPWM(ch, 0, 4096);
    else                   pwm_.setPWM(ch, 0, duty);

...
bool photodiode::init() {
  ...
  s_instance->adc->adc0->enableInterrupts(
      s_instance->interrupt_service_routine);
...
void photodiode::update() {
  ...
  if (microinterval >= 20) {
    ...
    if (s_instance->photo_diode_state == NOTREADY) {
      ...
      if (count == 10) {
        s_instance->ambient_light_threshold =
            s_instance->ambient_light_threshold / count;
        count = 0;
        s_instance->photo_diode_state = AWAITING;
      }
    } else if (s_instance->photo_diode_state ==
               AWAITING) {  // the 1500 can be changed currently don't have any
                            // teensys to mess with...
      if (s_instance->photo_diode_reading >=
          (s_instance->ambient_light_threshold + 1500)) {

 ...
 * @brief Pseudo-code Arm Subsystem for moving the robot arm via service interface.
 */
...
class ArmSubsystemSetup : public Classes::BaseSetup {
 public:
  ArmSubsystemSetup(const char* _id,
                    Robot::PCA9685Driver* servo_driver,
...
class ArmSubsystem : public IMicroRosParticipant,
 ...
 public:
  ...
  void update() override {
    ...
    // float pos = (setup_.encoder_) ? setup_.encoder_->getPosition().position : 0.0f;
    // translate `pos` into feedback for state machine
  ...
  void pause() override {
    state_ = mcu_msgs__msg__ArmSusbsytem__STOPPED;
  }
  void reset() override {
  ...
  bool onCreate(rcl_node_t* node, rclc_executor_t* executor) override {
    ...
    //     ROSIDL_GET_MSG_TYPE_SUPPORT(mcu_msgs, msg, ArmSusbsytem), "arm/state");

    // Pseudo-code: create service for `mcu_msgs/srv/ArmControl`
    // rclc_service_init_default(&srv_control_, node_, 
    //     ROSIDL_GET_SRV_TYPE_SUPPORT(mcu_msgs, srv, ArmControl), "arm/control");
  ...
  void handleServiceControl(const mcu_msgs__srv__ArmControl_Request* request,
    ...
    if (state_ != mcu_msgs__msg__ArmSusbsytem__ARMED && 
      ...
      // response->message = rosidl_runtime_c__String__assign(&response->message, "Arm not ready");
      return;
    }
    ...
    if (target_position_state_ == mcu_msgs__srv__ArmControl_Request__POSITION_CUSTOM) {
    ...
    // int16_t target_pos = getTargetPosition(target_position_state_, target_custom_position_);
    // uint16_t pwm_value = positionToPWM(target_pos);
 ...
 private:
  ...
  void updateMovementState() {
    if (position_state_ == mcu_msgs__msg__ArmSusbsytem__POSITION_MOVING) {
      ...
      if (elapsed >= setup_.movement_duration_ms_) {
        ...
        // int16_t expected = getTargetPosition(target_position_state_, target_custom_position_);
        // if (abs(actual_pos - expected) < POSITION_TOLERANCE_TICKS) {
        //   position_state_ = target_position_state_;
        // } else {
  ...
  //   const auto* req = static_cast<const mcu_msgs__srv__ArmControl_Request*>(reqin);
  //   auto* res = static_cast<mcu_msgs__srv__ArmControl_Response*>(resin);
  ...
  uint8_t target_position_state_ = mcu_msgs__msg__ArmSusbsytem__POSITION_RETRACTED;

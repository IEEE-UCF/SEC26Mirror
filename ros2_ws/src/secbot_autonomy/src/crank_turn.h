 ...
 * @file crank_turn.h
 * @author Rafeed Khan
 * @brief Antenna #2 crank task: rotate the crank at least 540 degrees using a small task state machine.
 *
 * This task DOES NOT move the drivetrain
 * It returns TankPwmCmd with left/right = 0 and uses IO callbacks to drive the crank actuator
 */
...
/**
 * @brief Spin the crank until we hit target rotation (prefer encoder), otherwise open-loop timed
 *
 * State machine:
...
class CrankTurnTask {
 public:
  struct Config {
    ...
    float target_deg = 540.0f;     ///< minimum degrees to rotate (rules say >= 540)

    // direction: +1 = CW, -1 = CCW (pick one and keep it consistent with the arm aldem)
    int8_t direction = +1;

    // actuator command (raw PWM to your crank motor / end-effector motor driver)
    int16_t spin_pwm = 180;        ///< magnitude to spin with (signed applied by direction)

    // if we somehow never make the encoder, we spin for time = target_deg / est_deg_per_s (or override)
    float est_deg_per_s = 180.0f;  ///< YOU CALIBRATE THIS ON REAL HARDWARE
    float open_loop_spin_s = -1.0f;///< if >0, use this exact time instead of computed

    // tolerances (only used when encoder exists SO PLEASE SOMOENE MAKE THE ENCODER)
  ...
  struct IO {
    ...
    // This is optional, read crank angle in degrees (0..360 or any wrap is fine)
    float (*read_crank_deg)(void) = nullptr;

    // ALSO OPTIONAL, if we have a “task complete” signal (vision, LED sensor, etc)

...
class keystroke_translator : public rclcpp::Node {
 public:
  keystroke_translator()
      : Node("keystroke_translator"),
        current_x_velocity(0.0),
        current_y_velocity(0.0),
        current_z_velocity(0.0) {
    // position_publisher =
    // this->create_publisher<geometry_msgs::msg::Pose>("drone_position",10);
    velocity_publisher =
        this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);
    auto keystroke_callback =
        [this](std_msgs::msg::Char::UniquePtr received_msg)
        -> void {  // callback function for subscriber
      ...
      switch (received_msg->data) {
        case ('w'):
          ...
          RCLCPP_INFO(this->get_logger(), "Position x now: %f",
                      current_x_velocity);
        ...
        case ('a'):
          ...
          RCLCPP_INFO(this->get_logger(), "Position y now: %f",
                      current_y_velocity);
          velocity_message.linear.x = current_x_velocity;
          velocity_message.linear.y = current_y_velocity;
          velocity_message.linear.z = current_z_velocity;
        ...
        case ('i'):
          ...
          RCLCPP_INFO(this->get_logger(), "Position z now: %f",
                      current_z_velocity);
          velocity_message.linear.x = current_x_velocity;
          velocity_message.linear.y = current_y_velocity;
          velocity_message.linear.z = current_z_velocity;
    ...
    keystroke_subscriber = this->create_subscription<std_msgs::msg::Char>(
        "keystroke_topic", 10, keystroke_callback);
  }
 ...
 private:
  ...
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr
      velocity_publisher;  // seems like the only way we can move in this sim
};
/*
if you want to double check if your bridge is a ok run the simulation FIRST(yes
this means launching the sdf world and the drone) then run these commands
...
as for me i am just checking to see if my bridge obtained the correct topic name
and that gazebo has a publisher(ROS2) and a subscriber(gazebo itself)
  */
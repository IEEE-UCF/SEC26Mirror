...
UWB_Tag_Sim::UWB_Tag_Sim() : Node("UWB_Tag_Simulation_Node") {
  ...
  std::string world_topic =
      "/world/car_world/pose/info";  // this can be changed check gz topic -l
  if (!subscription_to_world.Subscribe(
          world_topic, &UWB_Tag_Sim::gazeboworldstats_executor_callback,
          this)) {
    RCLCPP_ERROR(this->get_logger(), "Failed to subscribe to the world file!");
  }
  uwb_ranges_array_publisher =
      this->create_publisher<mcu_msgs::msg::UWBRanging>(
          "/" + tag_name + "/ranging", 10);
  using namespace std::chrono_literals;
  wall_timer = this->create_wall_timer(
      30ms, std::bind(&UWB_Tag_Sim::rostwo_executor_callback, this));
}
void UWB_Tag_Sim::gazeboworldstats_executor_callback(
...
void UWB_Tag_Sim::rostwo_executor_callback() {
  ...
  // Pose v is an array of pose information however, each pose information is
  // stuck to a tag whatever tag you want to find you need to change the
  // tag_name
  ...
  for (int i = 0; i < current_data.pose_size(); i++) {
    ...
    if ((name.find("anchor") != std::string::npos)) {
      ...
      double simulated_distance =
          distance + distribution_normal(tandomGenerator);
      ...
      if (name.find("anchor_10") != std::string::npos) {
        range_info.distance =
            simulated_distance;  // might be published in meters need to convert
                                 // to centimeters

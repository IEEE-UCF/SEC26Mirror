...
class DStarLite {
 public:
  ...
  void
  compute_shortest_path();  // computes the shortest path, used in re planning
  std::vector<Node> extract_path();  // show the path

 private:
  double distance_between_points(
      const Node &a, const Node &b) const;  // find distance between 2 nodes
  Priority calculate_priority(const Node &s) const;  // find the cost so far
  double cost(const Node &a,
              const Node &b) const;  // finds if node is free on the map
  ...
  Node start_;
  Node goal_;
  double offset_old_poriority_;  // also known as km which is just lazily given
                                 // the old priority (HEURISTICS)

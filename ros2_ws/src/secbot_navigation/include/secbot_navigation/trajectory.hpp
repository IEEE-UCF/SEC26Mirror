...
class PurePursuitController {
 public:
  ...
  std::pair<double, double> vector_projection(
      std::pair<double, double> A, std::pair<double, double> B,
      std::pair<double, double> P);  // Closest point to robot P on line seg AB

  TrajectoryPoint find_lookahead_point(
      double robot_x,
      double robot_y);  // finds the next "carrot" for the robot to go to

  double linear_slowdown(
      double desired_v,
      double dist_to_goal);  // reduces speed as you get closer to the goal

  std::pair<double, double> coordinate_transform(
      std::tuple<double, double, double> robot_pose,
      std::pair<double, double>
          target_point);  // (WHERE ROBOT IS FACING) converts robots global
                          // target points to local points for the robot to
                          // understand

  std::pair<double, double> compute_curvature_and_velocity(
      std::pair<double, double> local_point,
      double current_v);  // (CONTROLS THE ANGUALR VELOCITY) computes the
                          // turning curvature and angular speed needed to reach
                          // the next lookahead point

  std::pair<double, double> differential_drive_kinematics(
      double v, double w);  // Controls how fast each wheel should spin

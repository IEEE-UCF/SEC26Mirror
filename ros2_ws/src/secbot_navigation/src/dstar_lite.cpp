...
DStarLite::DStarLite(GridMap &grid_map, Node start, Node goal,
    ...
    : map_(grid_map), start_(start), goal_(goal), offset_old_poriority_(0.0) {
  (void)config;
  ...
  g_.assign(rows,
            std::vector<double>(
                cols, INF));  // this is the cost from the node to the goal
  rhs_.assign(rows, std::vector<double>(
                        cols, INF));  // this is the lookahead cost (rhs(s) =
                                      // min(cost(s, s') + g(s')))
...
DStarLite::Priority DStarLite::calculate_priority(const Node &s) const {
  ...
  // min(g, rhs) known aass cost to go from s -> goal (biases search towards
  // nodes closer to the robot)
  double min_val = std::min(g_[s.r][s.c], rhs_[s.r][s.c]);

  // lower k1 higher priority           k2 prevents oscillation (stability)
  return {min_val + distance_between_points(start_, s) + offset_old_poriority_,
          min_val};
}
...
void DStarLite::update_start(Node start) {
  if (start != start_) {
    // Increment offset_old_poriority (priority modifier) to account for robot
    // movement This avoids recomputing prioritys for the entire graph.
    offset_old_poriority_ += distance_between_points(
        start_, start);  // this is HUGE ( when the robot moves the distance
                         // changes - instead of recomputing D* Lite shifts
                         // priority to offset_old_poriority_ also known as km)
    start_ = start;
  }
}

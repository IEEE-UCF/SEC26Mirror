...
class PathingNode : public rclcpp::Node {
 public:
  PathingNode() : Node("pathing_node") {
    ...
    debug_pub_ = this->create_publisher<visualization_msgs::msg::MarkerArray>(
        "/nav_debug", 1);  // placeholder for markers
    pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
        "/nav_pose", 10);  // placeholder for publishing pose
 ...
 private:
  ...
  rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr
      path_pub_;  // Path publisher for RViz
  rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr
      debug_pub_;  // MarkerArray publisher for debug visualization
  ...
  // === helper ===
  void log_state(const char *tag) { RCLCPP_INFO(this->get_logger(), tag); }
  ...
  std::unique_ptr<PurePursuitController>
      controller_;  // pure pursuit (THE RAFEED ALGORITM)
  ...
  std::string load_filepath(const std::string &folder,
                            const std::string &filename) {
    return ament_index_cpp::get_package_share_directory("secbot_navigation") +
           folder + filename;
  }
  ...
  void load_config() {
    ...
    std::string nav_path = load_filepath(
        "/config/", this->get_parameter("config_file").as_string());
    std::string arena_path = load_filepath(
        "/config/", this->get_parameter("arena_file").as_string());
    ...
    try {
      ...
      if (nav_cfg["smoothing"]["max_skip"])
        max_skip_ =
            nav_cfg["smoothing"]["max_skip"].as<double>();  // add max skip
  ...
  void init_planner() {
    ...
    // Set Goal
    int gr = static_cast<int>(
        (goal_y_ - origin_.second) /
        resolution_);  // Sets the goal on map using resolution to guide it
  ...
  void goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Replanned to goal (%.2f, %.2f) from (%.2f, %.2f)", goal_x_,
                goal_y_, robot_x_, robot_y_);
  }
  ...
  void control_loop() {
    ...
    if (!state_received_ || !current_trajectory_) {
      RCLCPP_INFO(this->get_logger(),
                  "Skipping control loop (state_received=%d, trajectory=%d)",
                  state_received_ ? 1 : 0, current_trajectory_ ? 1 : 0);
      if (!state_received_) {
        log_state(
            "Not recieved /odom command to start (please call in another "
            "terminal)");
      }
      if (!current_trajectory_) {
        log_state("No trajecotry to follow");
      }
    ...
    RCLCPP_INFO(this->get_logger(), "Found carrot at (x=%.2f, y=%.2f), v=%.2f",
                carrot.x, carrot.y, carrot.v);
    ...
    RCLCPP_INFO(this->get_logger(),
                "Wheel speeds set at (left=%.3f, right=%.3f",
                wheel_speeds.first, wheel_speeds.second);

    // Convert Wheel Speeds to Twist (v, w)
    auto cmd_msg = geometry_msgs::msg::Twist();  // publish to /cmd_vel

    cmd_msg.linear.x =
        (wheel_speeds.second + wheel_speeds.first) / 2.0;  // velocity
    cmd_msg.angular.z = (wheel_speeds.second - wheel_speeds.first) /
                        controller_->get_track_width();  // angular velocity
  ...
  void publish_and_print_path(const Trajectory &traj) {
    ...
    int idx = 0;
    for (const auto &p : traj) {
      RCLCPP_INFO(this->get_logger(), "Point %d: (%.2f, %.2f) v=%.2f", idx++,
                  p.x, p.y, p.v);
    }
    log_state("===========================");
  }

...
// Constructor stores params (track_width, lookahead_dist, max_v, max_w,
// slow_dist)
...
std::pair<double, double> PurePursuitController::vector_projection(
...
double PurePursuitController::linear_slowdown(double desired_v,
                                              double dist_to_goal) {
  if (slow_dist_ <= 0) return desired_v;
  return desired_v * std::min(dist_to_goal / slow_dist_, 1.0);
}

// === Converts the global target points to robots local frame (WHERE ROBOT IS
// FACING) ===
std::pair<double, double> PurePursuitController::coordinate_transform(
  ...
  // x = (target_x - robot_x) * cos(robot_theta) + (target_y - robot_y) *
  // sin(robot_theta)
  double x_local = dx * std::cos(std::get<2>(robot_pose)) +
                   dy * std::sin(std::get<2>(robot_pose));

  // y = -(target_x - robot_x) * cos(robot_theta) + (target_y - robot_y) *
  // sin(robot_theta)
  double y_local = -dx * std::sin(std::get<2>(robot_pose)) +
                   dy * std::cos(std::get<2>(robot_pose));

  return {x_local, y_local};
}

// === computes pure pursuit curavture and applies angular velocty limits to
// reach the next lookahead point "carrot" ===
std::pair<double, double> PurePursuitController::compute_curvature_and_velocity(
  ...
  double L2 = local_point.first * local_point.first +
              local_point.second * local_point.second;
...
std::pair<double, double> PurePursuitController::differential_drive_kinematics(
    double v, double w) {
...
std::pair<double, double> PurePursuitController::compute_control(
  ...
  // 1. Linear Slowdown
  double dist_to_goal =
      std::sqrt(std::pow(std::get<0>(robot_pose) - goal_point.first, 2) +
                std::pow(std::get<1>(robot_pose) - goal_point.second, 2));
  ...
  return differential_drive_kinematics(
      v_cmd, w_cmd);  // returns how fast the wheels should turn
}

...
: beacon_id_(beacon_id),
  ...
  description_(description.empty() ? "Beacon " + std::to_string(beacon_id) : description)
{
}

bool BeaconConfig::isAxisKnown(const std::string & axis) const
{
  return std::find(known_axes_.begin(), known_axes_.end(), axis) != known_axes_.end();
}
...
: tag_id_(tag_id),
  description_(description.empty() ? "Tag " + std::to_string(tag_id) : description),
  publish_topic_(publish_topic.empty() ? "/uwb/pose/tag_" + std::to_string(tag_id) : publish_topic),

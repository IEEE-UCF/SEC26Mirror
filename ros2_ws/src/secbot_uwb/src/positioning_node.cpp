...
: Node("uwb_positioning_node")
{
  ...
  declareParameters();
  loadConfiguration();
  setupMovingBeaconSubscriptions();

  // Create ranging subscription
  ranging_sub_ = this->create_subscription<mcu_msgs::msg::UWBRanging>(
    "mcu_uwb/ranging",
    10,
    std::bind(&UWBPositioningNode::rangingCallback, this, std::placeholders::_1));
...
void UWBPositioningNode::loadConfiguration()
{
  ...
  outlier_threshold_ = this->get_parameter("positioning.outlier_threshold").as_double();

  // Load covariance parameters
  base_xy_variance_ = this->get_parameter("covariance.base_xy_variance").as_double();
  base_z_variance_ = this->get_parameter("covariance.base_z_variance").as_double();
  scale_with_residual_ = this->get_parameter("covariance.scale_with_residual").as_bool();
  ...
  for (int beacon_id : beacon_ids) {
    ...
    try {
      ...
      BeaconType type = (type_str == "moving") ? BeaconType::MOVING : BeaconType::STATIONARY;
      ...
      if (this->has_parameter(prefix + "odometry_topic")) {
        odometry_topic = this->get_parameter(prefix + "odometry_topic").as_string();
      }
      if (this->has_parameter(prefix + "use_odometry_fusion")) {
        use_odometry_fusion = this->get_parameter(prefix + "use_odometry_fusion").as_bool();
      }
      ...
      beacons_.emplace(
        beacon_id,
        BeaconConfig(
          beacon_id, type, position, known_axes,
          odometry_topic, use_odometry_fusion, description));

    } catch (const std::exception & e) {
      RCLCPP_ERROR(
        this->get_logger(), "Error loading beacon %d: %s",
        beacon_id, e.what());
    }
  }
  ...
  for (int tag_id : tag_ids) {
    ...
    try {
      std::string description = this->get_parameter(prefix + "description").as_string();

      std::string publish_topic;
      if (this->has_parameter(prefix + "publish_topic")) {
        publish_topic = this->get_parameter(prefix + "publish_topic").as_string();
      }

      bool enable_3d = false;
      if (this->has_parameter(prefix + "enable_3d_positioning")) {
        enable_3d = this->get_parameter(prefix + "enable_3d_positioning").as_bool();
      }
      ...
      RCLCPP_ERROR(
        this->get_logger(), "Error loading tag %d: %s",
        tag_id, e.what());
    }
  }
}

void UWBPositioningNode::setupMovingBeaconSubscriptions()
{
  for (const auto & [beacon_id, beacon] : beacons_) {
    if (beacon.getType() == BeaconType::MOVING && beacon.useOdometryFusion()) {
      if (!beacon.getOdometryTopic().empty()) {
        // Create subscription for moving beacon odometry
        auto callback = [this, beacon_id](
          const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg) {
            this->movingBeaconCallback(msg, beacon_id);
          };

        odometry_subs_[beacon_id] =
          this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          beacon.getOdometryTopic(), 10, callback);

        RCLCPP_INFO(
          this->get_logger(),
          "Subscribed to moving beacon %d odometry: %s",
          beacon_id, beacon.getOdometryTopic().c_str());

        // Initialize with configured position
        moving_beacon_positions_[beacon_id] = beacon.getPosition();
      }
    }
  }
}
...
void UWBPositioningNode::logConfiguration()
{
  ...
  for (const auto & [beacon_id, beacon] : beacons_) {
    std::string type_str = (beacon.getType() == BeaconType::STATIONARY) ? "stationary" : "moving";
    auto pos = beacon.getPosition();
    RCLCPP_INFO(
      this->get_logger(),
      "  Beacon %d (%s): pos=[%.2f, %.2f, %.2f]",
  ...
  for (const auto & [tag_id, tag] : tags_) {
    RCLCPP_INFO(
      this->get_logger(),
      "  Tag %d: %s, 3D=%s",
      tag_id, tag.getDescription().c_str(), tag.enable3D() ? "true" : "false");
  }

  RCLCPP_INFO(this->get_logger(), "========================");
}

void UWBPositioningNode::movingBeaconCallback(
  const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg,
  int beacon_id)
{
  ...
  std::array<double, 3> odom_position = {
    msg->pose.pose.position.x,
    msg->pose.pose.position.y,
    msg->pose.pose.position.z
  };

  // Update position based on known axes
  std::array<double, 3> updated_position = beacon.getPosition();

  if (!beacon.isAxisKnown("x")) {
    updated_position[0] = odom_position[0];
  }
  if (!beacon.isAxisKnown("y")) {
    updated_position[1] = odom_position[1];
  }
  if (!beacon.isAxisKnown("z")) {
    updated_position[2] = odom_position[2];
  }

  // Keep known axes fixed
  if (beacon.isAxisKnown("z")) {
    updated_position[2] = beacon.getPosition()[2];
  }

  moving_beacon_positions_[beacon_id] = updated_position;
}

std::array<double, 3> UWBPositioningNode::getBeaconPosition(int beacon_id) const
{
  auto it = beacons_.find(beacon_id);
  if (it == beacons_.end()) {
    return {0.0, 0.0, 0.0};
  ...
  if (beacon.getType() == BeaconType::STATIONARY) {
    return beacon.getPosition();
  } else {  // MOVING
    auto pos_it = moving_beacon_positions_.find(beacon_id);
    if (pos_it != moving_beacon_positions_.end()) {
...
void UWBPositioningNode::rangingCallback(const mcu_msgs::msg::UWBRanging::SharedPtr msg)
{
...
bool UWBPositioningNode::trilaterate(
  ...
  const TagConfig & tag_config)
{
  ...
  if (pose_publishers_.find(tag_id) == pose_publishers_.end()) {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Created publisher for tag %d (%s) on %s",
      tag_id, tag_config.getDescription().c_str(), tag_config.getPublishTopic().c_str());
  }

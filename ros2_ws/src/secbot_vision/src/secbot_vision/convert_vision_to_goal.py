...
class ConvertVisionToGoal(Node):
    def __init__(self):
        ...
        # fallback camera intrinsics (used when no CameraInfo topic is available)
        # defaults are for a typical 640x480 webcam (~60° horizontal FOV)
        self.declare_parameter("image_width", 640)
        self.declare_parameter("image_height", 480)
        self.declare_parameter("camera_fx", 0.0)   # 0 = auto-compute from image_width
        self.declare_parameter("camera_fy", 0.0)
        self.declare_parameter("camera_cx", 0.0)   # 0 = image_width / 2
        ...
        # state — apply fallback intrinsics if provided ========
        img_w = int(self.get_parameter("image_width").value)
        img_h = int(self.get_parameter("image_height").value)
        param_fx = float(self.get_parameter("camera_fx").value)
        param_fy = float(self.get_parameter("camera_fy").value)
        param_cx = float(self.get_parameter("camera_cx").value)
        param_cy = float(self.get_parameter("camera_cy").value)

        # auto-compute reasonable defaults from image size (~60° HFOV)
        default_fx = float(img_w) * 0.9   # ≈ 576 for 640px
        default_fy = default_fx
        default_cx = float(img_w) / 2.0
        default_cy = float(img_h) / 2.0

        self.fx = param_fx if param_fx > 0.0 else default_fx
        self.fy = param_fy if param_fy > 0.0 else default_fy
        self.cx = param_cx if param_cx > 0.0 else default_cx
        self.cy = param_cy if param_cy > 0.0 else default_cy

        self.get_logger().info(
            f"Using fallback intrinsics: fx={self.fx:.1f} fy={self.fy:.1f} "
            f"cx={self.cx:.1f} cy={self.cy:.1f} (from {img_w}x{img_h})")
        self.get_logger().info("CameraInfo topic will override these if received.")
    ...
    def on_camera_info(self, msg: CameraInfo):
        ...
        self.get_logger().info(
            f"CameraInfo received — overriding intrinsics: "
            f"fx={self.fx:.2f}, fy={self.fy:.2f}, cx={self.cx:.2f}, cy={self.cy:.2f}", once=True)

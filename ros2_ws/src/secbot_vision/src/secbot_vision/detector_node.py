...
class DetectorNode(Node):
    ...
    def _on_color_tunning(self, msg: ColorTunning):
        ...
        """
        # GUI sends hue 0-360°, OpenCV HSV uses 0-179
        h = int(msg.hue * 179 / 360)
        s, v = msg.saturation, msg.value
    ...
    def on_image(self, msg: Image):
        ...
        debug = frame.copy()
        best_mask = np.zeros(frame.shape, dtype=np.uint8)
...
def process_frame(frame, color_low, color_high, hsv_values, min_area_ratio=0.0002):
    ...
    for contour in contours:
        ...
        if yy.size > 0:
            ...
            hue_i = np.maximum(0.0, 1.0 - (d_i / max(saturation, 1))).mean()

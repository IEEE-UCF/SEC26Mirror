...
double angle_normalize(double angle) { return angle >= 0 ? angle : angle + PI; }

bool fequal(double f1, double f2) { return fabs(f1 - f2) < EPS; }

LOCATOR_RETURN calculate_pos_robust_ransac(
    std::unordered_map<int, Position3D> anchorPoseMap,
    std::unordered_map<int, double> distances, Position3D &res) {
  ...
  if (distances.size() < CALCULABLE_LEAST_CNT) {
    ...
    for (int i = 0; i < max_iterations; i++) {
      ...
      Position3D res_temp;
      if (CALCULATE_SUCCESS == calculate_pos_with_index_(anchorPoseMap,
                                                         distances,
                                                         sample_indexes, res)) {
        ...
        get_all_fit_indexes_(anchorPoseMap, distances, sample_indexes, res_temp,
                             threshold, all_fit_indexes, total_residual);

        if (fit_success == false ||
            all_fit_indexes.size() > all_fit_indexes_best.size() ||
            (all_fit_indexes.size() == all_fit_indexes_best.size() &&
             total_residual < residual_best)) {
    ...
    // refine model
    if (fit_success) {
      if (ENABLE_RANSAC_REFINE &&
          all_fit_indexes_best.size() >= RANSAC_LEAST_CNT) {
        Position3D res_temp;
        if (CALCULATE_SUCCESS ==
            calculate_pos_with_index_(anchorPoseMap, distances,
                                      all_fit_indexes_best, res_temp)) {
          double residual_refined = 0.0;

          for (int i = 0; i < all_fit_indexes_best.size(); i++) {
            double estimated_distance = sqrt(
                pow((anchorPoseMap[all_fit_indexes_best[i]].x - res.x), 2) +
                pow((anchorPoseMap[all_fit_indexes_best[i]].y - res.y), 2));
            double difference_of_distances =
                fabs(distances[all_fit_indexes_best[i]] - estimated_distance);
            residual_refined +=
                difference_of_distances * difference_of_distances;
          }

          if (residual_refined < residual_best) {
            // order sensitive, so residual_refined may be big than
            // residual_best
            res = res_temp;
          }
        }
...
LOCATOR_RETURN calculate_pos_for_all_(
    std::unordered_map<int, Position3D> anchorPoseMap,
    std::unordered_map<int, double> distances, Position3D &res) {
  ...
  return calculate_pos_with_index_(anchorPoseMap, distances,
                                   effective_landmark_indexes, res);
}

LOCATOR_RETURN calculate_pos_with_index_(
    std::unordered_map<int, Position3D> anchorPoseMap,
    std::unordered_map<int, double> distances,
    std::vector<int> effective_landmark_indexes, Position3D &res) {
  ...
  int index_last =
      effective_landmark_indexes[effective_landmark_indexes.size() - 1];
  for (int i = 0; i < effective_landmark_indexes.size() - 1; i++) {
    int index_i = effective_landmark_indexes[i];
    gsl_matrix_set(
        A, i, 0, (anchorPoseMap[index_i].x - anchorPoseMap[index_last].x) * 2);
    gsl_matrix_set(
        A, i, 1, (anchorPoseMap[index_i].y - anchorPoseMap[index_last].y) * 2);
    gsl_matrix_set(
        A, i, 2, (anchorPoseMap[index_i].z - anchorPoseMap[index_last].z) * 2);
  ...
  gsl_vector *tau =
      gsl_vector_alloc(coefficient_num);  // matrix_row_num, coefficient_num
  gsl_vector *residuals = gsl_vector_alloc(matrix_row_num);

  gsl_multifit_linear_workspace *w =
      gsl_multifit_linear_alloc(matrix_row_num, coefficient_num);
  gsl_multifit_linear_svd(A, w);
  double rcond = gsl_multifit_linear_rcond(w);  // reciprocal condition number

  if (rcond < 1e-4) {
    // conditional number is too large, indicating that the problem is
    // ill-conditioned.
...
bool is_landmarks_in_one_line(std::unordered_map<int, Position3D> anchorPoseMap,
  ...
  double angle_0 = angle_normalize(
      atan2(anchorPoseMap[index_0].y - anchorPoseMap[index_last].y,
            anchorPoseMap[index_0].x - anchorPoseMap[index_last].x));

  for (int i = 1; i < effective_landmark_indexes.size() - 1; i++) {
    int index_i = effective_landmark_indexes[i];
    double angle_i = angle_normalize(
        atan2(anchorPoseMap[index_i].y - anchorPoseMap[index_last].y,
...
void get_all_fit_indexes_(std::unordered_map<int, Position3D> anchorPoseMap,
                          ...
                          double threshold, std::vector<int> &all_fit_indexes,
  ...
  for (auto it : distances) {
    ...
    double estimated_distance = sqrt(pow((anchorPoseMap[id].x - p.x), 2) +
                                     pow((anchorPoseMap[id].y - p.y), 2));

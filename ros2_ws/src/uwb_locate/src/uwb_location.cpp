...
UWBLocation::UWBLocation() : Node("uwb_location") {
  ...
  std::string labelName = this->get_parameter("label_name")
                              .get_parameter_value()
                              .get<std::string>();
  ...
  RCLCPP_INFO(this->get_logger(), "subscribe topic : %s",
              subscribeTopicName.c_str());

  subscription_ = this->create_subscription<uwb_interfaces::msg::UWBData>(
      subscribeTopicName, 10,
      std::bind(&UWBLocation::topic_callback, this, std::placeholders::_1));

  std::string publishTopic = "/uwbLocationRes/" + labelName;
  msgPublisher_ =
      this->create_publisher<geometry_msgs::msg::Point>(publishTopic, 10);
  RCLCPP_INFO(this->get_logger(), "publish topic : %s", publishTopic.c_str());
}

void UWBLocation::topic_callback(
    const uwb_interfaces::msg::UWBData::SharedPtr msg) {
  ...
  calState =
      calculate_pos_robust_ransac(anchorPoseMap, uwbDistance, estimatedRes);
  if (calState == CALCULATE_SUCCESS) {
    // RCLCPP_INFO(this->get_logger(), "uwb location success. res: x: %f, y: %f,
    // z:%f ",
    ...
    RCLCPP_INFO(this->get_logger(), "uwb location failed (type: %d). ",
                calState);
  }
}
...
void UWBLocation::load_anchors_pos() {
  std::string packageShareDirectory =
      ament_index_cpp::get_package_share_directory("uwb_locate");

  std::string anchorConfigFilePath =
      packageShareDirectory + "/config/anchor.xml";
  ...
  while (anchor) {
    ...
    RCLCPP_INFO(this->get_logger(), "load anchor id: %d position:%f %f %f", id,
                anchorPoseMap[id].x, anchorPoseMap[id].y, anchorPoseMap[id].z);
